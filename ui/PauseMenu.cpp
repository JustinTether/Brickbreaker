...
void PauseMenu::Draw(Engine* Engine)
{
  ...
  CLAY({
    ...
    CLAY_TEXT(CLAY_STRING("Paused"), CLAY_TEXT_CONFIG({.textColor = PixelToClayColor(olc::WHITE), .fontSize = 24})); 
    CLAY({
    ...
    })
    {
      ...
      {
        CLAY_TEXT(CLAY_STRING("Resume"), CLAY_TEXT_CONFIG({.textColor = PixelToClayColor(olc::WHITE), .fontSize = 8})); 
        

        // Handle Mouse clicking the Start button
        if(Engine->GetMouse(0).bPressed && Clay_PointerOver(CLAY_ID("ResumeButton")))
        {
