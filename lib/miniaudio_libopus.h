...
#include "miniaudio.h"
  ...
  typedef struct
  {
    ma_data_source_base ds; /* The libopus decoder can be used independently as
                               a data source. */
    ma_read_proc onRead;
    ma_seek_proc onSeek;
    ma_tell_proc onTell;
  ...
  MA_API ma_result ma_libopus_init_file(
      ...
      const ma_allocation_callbacks* pAllocationCallbacks, ma_libopus* pOpus);
  MA_API void
  ma_libopus_uninit(ma_libopus* pOpus,
  ...
  MA_API ma_result ma_libopus_get_data_format(
      ma_libopus* pOpus, ma_format* pFormat, ma_uint32* pChannels,
